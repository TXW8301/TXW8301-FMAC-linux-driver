...
static int hgic_iwpriv_set_roaming_bssid(struct hgic_fwctrl *ctrl, u8 ifidx, char *data, u32 count)
{
    u8 mac[6];
    hgic_pick_macaddr(data, mac);
    hgic_err("roaming to BSS: %pM\r\n", mac);
    return hgic_fwctrl_set_roaming_bssid(ctrl, ifidx, mac);
}
static int hgic_iwpriv_set_bss_disabled(struct hgic_fwctrl *ctrl, u8 ifidx, char *data, u32 count)
{
    u8  addr[6];
    u32 dis = 0;
    ...
    hgic_pick_macaddr(data, addr);
    dis = simple_strtol(data + 18, 0, 10);
    return  hgic_fwctrl_set_bss_disabled(ctrl, ifidx, dis, addr);
}
static int hgic_iwpriv_set_kick_assoc(struct hgic_fwctrl *ctrl, u8 ifidx, char *data, u32 count)
{
    int val = simple_strtol(data, 0, 10);
    return hgic_fwctrl_set_kick_assoc(ctrl, ifidx, val);
}
static int hgic_iwpriv_set_start_assoc(struct hgic_fwctrl *ctrl, u8 ifidx, char *data, u32 count)
{
    int val = simple_strtol(data, 0, 10);
...
static struct fwctrl_cfgset hgpriv_sets[] = {
    ...
    {"roaming_bssid",               hgic_iwpriv_set_roaming_bssid},
...
static int hgic_iwpriv_get_link_quality(struct hgic_fwctrl *ctrl, u8 ifidx, struct iwreq *wrqin)
{
    int ret;
    char *ptr;
    char buff[64];
    char format = 0;
    struct hgic_link_quality info;
    char *print_buf = kzalloc(512, GFP_ATOMIC);
    ...
    ret = hgic_fwctrl_get_link_quality(ctrl, ifidx, &info);
    info.obss_stas   = le32_to_cpu(info.obss_stas);
    info.bss_stas    = le32_to_cpu(info.bss_stas);
    info.cca_busy[0] = le32_to_cpu(info.cca_busy[0]);
    info.cca_busy[1] = le32_to_cpu(info.cca_busy[1]);
    info.cca_busy[2] = le32_to_cpu(info.cca_busy[2]);
    info.cca_busy[3] = le32_to_cpu(info.cca_busy[3]);
    info.cca_busy[4] = le32_to_cpu(info.cca_busy[4]);
    info.cca_busy[5] = le32_to_cpu(info.cca_busy[5]);
    info.cca_busy[6] = le32_to_cpu(info.cca_busy[6]);    
    info.cca_busy[7] = le32_to_cpu(info.cca_busy[7]);
    if (format == 1) {
        hgic_copyto_iwreq(wrqin, (char *)&info, sizeof(info));
    } else {
        ret = sprintf(print_buf, "Link Quality: bgrssi=%d, rssi=%d, evm=%d, per=%d, obss_sta=%d, bss_stas=%d, cca=[%d,%d,%d,%d,%d,%d,%d,%d]\r\n",
                      (s8)info.bgrssi, (s8)info.rssi, (s8)info.evm, (s8)info.per, info.obss_stas, info.bss_stas, 
                      info.cca_busy[0], info.cca_busy[1], info.cca_busy[2], info.cca_busy[3],
                      info.cca_busy[4], info.cca_busy[5], info.cca_busy[6], info.cca_busy[7]);
        hgic_copyto_iwreq(wrqin, print_buf, ret);
    }
    kfree(print_buf);
    return (0);
