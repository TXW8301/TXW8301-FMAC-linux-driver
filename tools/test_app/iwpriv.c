...
int hgic_iwpriv_get_sta_list(char *ifname, struct hgic_sta_info *sta_buf, int buf_size)
{
    return hgic_iwpriv_get_bytes(ifname, "sta_list=1", (char *)sta_buf, buf_size);//output binary structure
}
...
int hgic_iwpriv_set_roaming_bssid(char *ifname, char *bssid)
{
    return hgic_iwpriv_set_mac(ifname, "roaming_bssid", bssid);
}
int hgic_iwpriv_get_link_quality(char *ifname, struct hgic_link_quality *link)
{
    return hgic_iwpriv_get_bytes(ifname, "link_quality=1", (char *)link, sizeof(struct hgic_link_quality));
}
int hgic_iwpriv_set_bss_disabled(char *ifname, int disable, char *bssid)
{
    char cmd[128];
    memset(cmd, 0, sizeof(cmd));

    if (strlen(bssid) == 17 && bssid[2] == ':' && bssid[5] == ':' && bssid[8] == ':' && bssid[11] == ':' && bssid[14] == ':') {
        sprintf(cmd, "%s set bss_disable=%s,%d", ifname, bssid, disable);
    } else {
        sprintf(cmd, "%s set bss_disable="MACSTR",%d", ifname, MACARG(bssid), disable);
    }
    return hgic_iwpriv_do(cmd, 0, 0, 0, 0);    
}

int hgic_iwpriv_set_kick_assoc(char *ifname, int enable)
{
    return hgic_iwpriv_set_int(ifname, "kick_assoc", enable);
}

int hgic_iwpriv_set_start_assoc(char *ifname, int start)
{
