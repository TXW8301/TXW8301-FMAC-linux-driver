...
int hgic_fwctrl_set_roaming_bssid(struct hgic_fwctrl *ctrl, u8 ifidx, u8 *bssid)
{
    return hgic_fwctrl_set_bytes(ctrl, ifidx, HGIC_CMD_SET_ROAMING_BSSID, bssid, 6);
}
int hgic_fwctrl_get_link_quality(struct hgic_fwctrl *ctrl, u8 ifidx, struct hgic_link_quality *link)
{
    return hgic_fwctrl_get_bytes(ctrl, ifidx, HGIC_CMD_GET_LINK_QUALITY, (u8 *)link, sizeof(struct hgic_link_quality));
}
int hgic_fwctrl_set_bss_disabled(struct hgic_fwctrl *ctrl, u8 ifidx, u8 dis, u8 *bssid)
{
    u8 buff[7];
    buff[0] = dis;
    memcpy(buff+1, bssid, 6);
    return hgic_fwctrl_set_bytes(ctrl, ifidx, HGIC_CMD_SET_DISABLE_BSS, buff, 7);
}
int hgic_fwctrl_set_kick_assoc(struct hgic_fwctrl *ctrl, u8 ifidx, u32 enable)
{
    return hgic_fwctrl_set_int_val(ctrl, ifidx, HGIC_CMD_SET_KICK_ASSOC, enable);
}
